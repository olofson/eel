...
int sfifo_init(sfifo_t *f, int size)
{
	 ...
	 *
	 * Bug, feature or weirdness - I dunno.
	 * If you need room for N bytes, the
	 * buffer must be at least N + 1 bytes.
	 * Otherwise, the FIFO can't tell 'empty'
	 * from 'full'. Please submit a patch if
	 * you have a nice solution for this. :-)
	 */
	f->size = 1;
...
int sfifo_write_user_spin(sfifo_t *f, const void *buf, int len)
{
	while(sfifo_space(f) < len)
		;
	return sfifo_write_user(f, buf, len);
}
...
int sfifo_read_user_spin(sfifo_t *f, void *buf, int len)
{
	while(sfifo_used(f) < len)
		;
	return sfifo_read_user(f, buf, len);
}
#endif
